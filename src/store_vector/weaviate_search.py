...
class WeaviateSearcher:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if not weaviate_url or not weaviate_api_key:
                logger.error("Missing Weaviate configuration. Please set WEAVIATE_URL and WEAVIATE_API_KEY")
                return False
            ...
            try:
                if hasattr(response, 'final_answer'):
                    ...
                    if hasattr(response, 'display'):
                        ...
                        logger.warning(f"Could not extract answer from response type: {type(response)}")
    ...
    def _extract_chunks_from_response(self, response) -> List[str]:
        ...
        try:
                    ...
                    if hasattr(search, 'objects') and search.objects:
                        ...
                        for j, obj in enumerate(search.objects):
                            if hasattr(obj, 'properties'):
                                ...
                                logger.info(f"Object {j} properties keys: {list(properties.keys())}")
            ...
            if not chunks and hasattr(response, 'sources') and response.sources:
                ...
                for i, source in enumerate(response.sources):
                    ...
                    if hasattr(source, 'object_id') and hasattr(source, 'collection'):
                        ...
                        logger.info(f"Source {i}: object_id={object_id}, collection={collection_name}")
                        ...
                        try:
                            ...
                            if obj and hasattr(obj, 'properties'):
                                properties = obj.properties
                                logger.info(f"Fetched object properties keys: {list(properties.keys())}")
                                # Look for text content in common property names
                                for prop_name in ['text', 'content', 'chunk', 'data']:
                                    if prop_name in properties:
                                        text_content = properties[prop_name]
            ...
            if not chunks:
                logger.warning("No chunks found in Query Agent response, using BM25 search as fallback")
                try:
                    ...
                    bm25_response = collection.query.bm25(
                        query=response.response.content[:100] if hasattr(response, 'response') and hasattr(response.response, 'content') else "pháp luật",
                        limit=3
                    )
