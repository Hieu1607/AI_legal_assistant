...
def load_questions_from_excel(file_path, limit=30):
    ...
    try:
        ...
        questions_column = df.columns[1] if len(df.columns) > 1 else df.columns[0]  # 'Nội dung câu hỏi'
        answers_column = df.columns[2] if len(df.columns) > 2 else None  # 'Đáp án chuẩn'
        
        questions_raw = df[questions_column].dropna().tolist()[:limit]
        answers_raw = df[answers_column].dropna().tolist()[:limit] if answers_column else []
...
class RAGUser(HttpUser):
    ...
    def rag_request(self):
        ...
        with self.client.post(
            ...
            result_data = {
                ...
                "total_time": round(query_time, 2)  # Use query_time as total_time initially
            }
            ...
            if response.status_code == 200:
                try:
                    ...
                    api_total_time = response_data.get("data", {}).get("timing", {}).get("total_time", 0)
                    
                    # Use API total time if available, otherwise use our measured time
                    result_data.update({
                        "rag_answer": rag_answer,
                        "total_time": round(api_total_time if api_total_time > 0 else query_time, 2)
                    })
                    
                    response.success()
                    logger.info(f"Question {self.question_index}/30 - Success - Total time: {result_data['total_time']:.2f}s")
                ...
                logger.error(f"Question {self.question_index}/30 - HTTP {response.status_code}")
            ...
            if completed_requests >= total_questions:
                logger.info(f"All {total_questions} questions completed. Stopping test...")
                self.environment.runner.quit()
    ...
    def check_completion(self):
        ...
        if completed_requests >= total_questions:
            logger.info(f"Completion check: {completed_requests}/{total_questions} completed. Stopping...")
            self.environment.runner.quit()
...
def save_results_to_excel_locust(results):
    ...
    try:
        ...
        df_filtered = df[['question', 'standard_answer', 'rag_answer', 'total_time']].copy()
        df_filtered.columns = [
            'Câu hỏi ban đầu',
            'Câu trả lời mẫu', 
            'Câu trả lời từ RAG',
        ...
        successful_requests = len([r for r in results if 'rag_answer' in r and not r['rag_answer'].startswith('HTTP Error') and not r['rag_answer'].startswith('JSON Parse Error')])
        avg_query_time = sum([r.get("total_time", 0) for r in results]) / total_requests if total_requests > 0 else 0
        ...
        # Fallback to CSV
        try:
            csv_file = f"rag_sequential_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def main_standalone():
    ...
    print("locust -f quick_test.py --headless --users=30 --spawn-rate=2 --run-time=300s --host=https://ai-legal-assistant-zswt.onrender.com")
