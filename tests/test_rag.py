...
# This allows Python to find the app module correctly
...
from app.rag import ask_LLM, get_relevant_sentences, router
...
def test_get_relevant_sentences(mock_search):
    # Sửa lại để patch đúng đường dẫn như được import trong app/rag.py
    with patch(
...
async def test_ask_model_good_case(client, mock_search):
    with (
        ...
        assert response["data"]["context_count"] == 5  # Kiểm tra số lượng context
